...
# is regenerated.
# ----------------------------------------------------------------------------------
param([switch]$Isolated, [switch]$Live, [switch]$Record, [switch]$Playback, [switch]$RegenerateSupportModule, [switch]$UsePreviousConfigForRecord, [string[]]$TestName, [string]$filePath)
...
#Import correct Az-Context
Import-AzContext -Path(Join-Path $filePath 'context.json')
...
. ("$filePath\test\pipeline-utils.ps1") -filePath $filePath
...
$localModulesPath = Join-Path $filePath 'generated\modules'
if(Test-Path -Path $localModulesPath)
{
  $env:PSModulePath = "$localModulesPath$([IO.Path]::PathSeparator)$env:PSModulePath"
}

$modulePsd1 = Get-Item -Path (Join-Path $filePath './Az.DesktopVirtualization.psd1')
...
. (Join-Path $filePath 'test\loadEnv.ps1') -filePath (Join-Path $filePath 'test')

$testFolder = $filePath
Invoke-Pester -Path "$testFolder\*" -EnableExit -OutputFile (Join-Path $testFolder "$moduleName-TestResults.xml")
