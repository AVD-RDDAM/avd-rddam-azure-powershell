if(($null -eq $TestName) -or ($TestName -contains 'Update-AzWvdConnectionPolicy'))
{
  ...
  . ($loadEnvPath)
  $TestRecordingFile = Join-Path $PSScriptRoot 'Update-AzWvdConnectionPolicy.Recording.json'
...
Describe 'Update-AzWvdConnectionPolicy' {
    It 'UpdateExpanded' {
        try{
            ...
            $connectionPolicy = Update-AzWvdConnectionPolicy -SubscriptionId $env.SubscriptionId `
                                -ResourceGroupName $env.ResourceGroup `
                                -Name $env.ConnectionPolicy `
            ...
            $connectionPolicy = Get-AzWvdConnectionPolicy -SubscriptionId $env.SubscriptionId `
                                -ResourceGroupName $env.ResourceGroup `
                                -Name $env.ConnectionPolicy
                $connectionPolicy.AuthenticationContext | Should -Be ''
                $connectionPolicy.Description | Should -Be "connectionPolicyDescription"
        }
        finally{
